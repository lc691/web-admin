...
# ===========================
@router.get("/data")  # ← Ubah dari /songs/data ke /data karena router sudah punya prefix /songs
def songs_data(
    request: Request,
    draw: int = Query(1),  # ← Default value
    start: int = Query(0, ge=0),
    length: int = Query(10, ge=1, le=100),
    search: Optional[str] = Query(None),  # ← Ubah jadi Optional
    status: Optional[str] = Query(None),
    channel_id: Optional[int] = Query(None),  # ← Ubah jadi int
    order_column: int = Query(0, alias="order[0][column]"),  # ← Default 0
    ...
    # Ordering - Map column index ke field
    order_columns = {
        0: "s.id",
        1: "s.id",
        2: "a.name",
